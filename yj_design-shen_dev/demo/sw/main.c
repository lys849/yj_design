...
//
// Fixes:
//   B5: keyboard_read now uses a simple delay instead of the deadlock-causing
//       "wait for release" loop. Hardware auto-clears the valid bit after ~1ms.
//   B8: flow_payment now sets payment amount directly instead of calling
//       the digit-by-digit function with a multi-digit value.
#include "config.h"
...
static void delay_ms(uint32_t ms) {
    // Rough delay: ~50M cycles per second on MicroBlaze at 50MHz
    // Each loop iteration is ~3-5 instructions → ~10ns per iter
...
// FIXED B5: removed deadlock "while (REG_KEYBOARD & 0x10)" loop.
// Hardware auto-clears the keyboard valid bit after ~1ms hold time.
static uint8_t keyboard_read(void) {
    ...
    while (1) {
        ...
        if ((val & 0x10) && (val != prev)) {
            key = val & 0x0F;
            // FIXED B5: simple delay instead of deadlock wait-for-release
            delay_ms(150);  // debounce: wait 150ms to avoid key repeat
...
static void flow_payment(void) {
    ...
    if (input_number(&amount_yuan) != 0) {
        display_status(10, (const uint8_t*)"Payment cancelled.");
        keyboard_read();
        return;
    }

    // FIXED B8: set payment amount directly (in cents)
    payment_set_amount(amount_yuan * 100);
