...
//
// Fixes:
//   B4: fp_send_command now checks status at bits 31:24 (was checking 31:24
//       which was always 0 due to old register layout).
//       fp_search parameter extraction now uses correct bit positions.
#include "config.h"

// ============================================
// Low-level: send command to fingerprint sensor
// ============================================
// FIXED B4: properly polls fp_status at bits 31:24 of REG_FP_RESP.
// Status values: 0=idle, 1=busy, 2=done(success), 3=error.
// Returns: 0 on success, -1 on timeout, -2 on sensor error.
static int fp_send_command(uint8_t cmd, uint16_t param) {
    ...
    // Wait for command to complete — poll status field at bits 31:24
    uint32_t timeout = 1000000;
    while (timeout--) {
        uint32_t resp = REG_FP_RESP;
        // FIXED B4: extract status from bits 31:24
...
int fp_match(uint16_t* score) {
    ...
    if (ret == 0) {
        // FIXED: response is now {fp_status, 8'd0, fp_response[15:0]}
        // fp_response[7:0] contains match score
        uint32_t resp = REG_FP_RESP;
...
int fp_search(uint16_t* page_id, uint16_t* score) {
    ...
    if (ret == 0) {
        // FIXED B4: response layout {fp_status, 8'd0, fp_response[15:0]}
        // fp_response[15:8] = page_id, fp_response[7:0] = score
        uint32_t resp = REG_FP_RESP;
