...
// FIXED: column calculation rewritten for correctness.
// The old code's while(yuan>=10) loop gave N-1 advances for N-digit numbers,
// causing the decimal point to overwrite the last digit.
void display_put_amount(int row, int col, uint32_t cents) {
    ...
    // FIXED: count digits of yuan to advance column correctly
    {
        uint32_t n = yuan;
        if (n == 0) {
            col += 1;  // single digit '0'
        } else {
            while (n > 0) { col++; n /= 10; }
        }
    }
